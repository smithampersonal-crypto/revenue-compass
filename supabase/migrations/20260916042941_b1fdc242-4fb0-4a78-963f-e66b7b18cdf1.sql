-- Phase 9F acceptance patch: restore the accepted Phase 8B semantics of
-- arc_commit_source_document_upload and re-layer only the intended Phase 9F
-- AI source-lifecycle hooks on top of them.
--
-- Regression being fixed: the Phase 9F rewrite (a) used bare `source_document_id`
-- and bare `lock_version` inside a RETURNS TABLE function whose OUT parameters
-- carry those exact names (ambiguous column reference, 42702), and (b) dropped
-- the accepted finalized-intent readback for guest and library-only intents.
--
-- Signature, volatility, security, search_path and grants are unchanged.
