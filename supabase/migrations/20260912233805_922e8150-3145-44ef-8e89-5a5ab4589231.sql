-- Phase 8A final lifecycle consistency patch.

-- 1. Attach is a genuine no-op when the document is already selected.
...
begin
  ...
  -- The stale-lock check happens before the mutation is evaluated, even when
...
-- 2. Remove is a genuine no-op when the association is already absent.
...
-- 3. Guest auto-selection after commit follows the same no-op principle.
...
-- 4. Deleting a guest-owned document requires a still-valid guest credential.
...
revoke all on function public.arc_attach_source_document(uuid,uuid,uuid,integer)
  from public, anon, authenticated;
revoke all on function public.arc_remove_source_document(uuid,uuid,uuid,integer)
  from public, anon, authenticated;
...
revoke all on function public.arc_stage_source_document_deletion(uuid,text,uuid,integer)
  from public, anon, authenticated;
grant execute on function public.arc_attach_source_document(uuid,uuid,uuid,integer) to service_role;
grant execute on function public.arc_remove_source_document(uuid,uuid,uuid,integer) to service_role;
