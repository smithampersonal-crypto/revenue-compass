...
begin
  if p_owner_user_id is null or p_contract_id is null then
    raise exception 'owner and contract are required' using errcode = '22023';
  end if;
...
revoke all on function public.arc_start_amendment_revision(uuid, uuid) from public;
revoke all on function public.arc_start_amendment_revision(uuid, uuid) from anon;
revoke all on function public.arc_start_amendment_revision(uuid, uuid) from authenticated;
grant execute on function public.arc_start_amendment_revision(uuid, uuid) to service_role;