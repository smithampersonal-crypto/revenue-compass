...
begin
  ...
  values (repeat('a', 64), draft, 'arc.workflow.v1', now() + interval '8 hours')
  returning id into fresh_guest;
  insert into public.guest_workspaces (token_hash, draft_json, schema_version, expires_at)
  values (repeat('b', 64), draft, 'arc.workflow.v1', now() - interval '1 hour')
  returning id into old_guest;
  insert into public.guest_workspaces (token_hash, draft_json, schema_version, expires_at)
  values (repeat('c', 64), draft, 'arc.workflow.v1', now() - interval '2 hours')
  returning id into empty_guest;
  insert into public.guest_workspaces (token_hash, draft_json, schema_version, expires_at,
                                       status, migrated_user_id)
  values (repeat('d', 64), draft, 'arc.workflow.v1', now() - interval '3 hours',
          'migrated', owner_id)
  ...
  values (old_guest, 'documents/guest-owned.pdf', 'g.pdf', 'Guest PDF', repeat('1', 64), 100, 1)
  returning id into guest_doc;
  insert into public.source_documents (contract_id, storage_object_path, original_filename,
                                       display_name, sha256, byte_size, page_count)
  values (contract_id, 'documents/migrated.pdf', 'm.pdf', 'Migrated PDF', repeat('2', 64), 100, 1)
  returning id into migrated_doc;
  insert into public.source_documents (contract_id, storage_object_path, original_filename,
                                       display_name, sha256, byte_size, page_count)
  values (contract_id, 'documents/finalized-upload.pdf', 'f.pdf', 'Final PDF', repeat('3', 64), 100, 1)
  ...
  values (fresh_guest, 'pending/fresh.pdf', 'fresh.pdf', 'Fresh', 'pending', now() + interval '50 minutes')
  returning id into fresh_intent;
  insert into public.document_upload_intents (guest_workspace_id, pending_object_path,
        permanent_object_path, original_filename, display_name, state, expires_at)
  values (fresh_guest, 'pending/stale.pdf', 'documents/never-committed.pdf',
          'stale.pdf', 'Stale', 'prepared', now() - interval '2 hours')
  returning id into stale_intent;
  insert into public.document_upload_intents (guest_workspace_id, pending_object_path,
        original_filename, display_name, state, expires_at)
  values (fresh_guest, 'pending/failed.pdf', 'failed.pdf', 'Failed', 'failed', now() - interval '3 hours')
  ...
  values (contract_id, 'pending/finalized.pdf', 'documents/finalized-upload.pdf', final_doc,
  ...
  select '12 fresh upload intent is untouched',
     ...
     and not exists (select 1 from public.storage_deletion_queue
                     where storage_object_path = 'pending/fresh.pdf');
  ...
  select '13 expired abandoned intent is terminalized and its pending object queued',
     ...
     and exists (select 1 from public.storage_deletion_queue
                 where storage_object_path = 'pending/stale.pdf' and completed_at is null)
     and exists (select 1 from public.storage_deletion_queue
                 where storage_object_path = 'documents/never-committed.pdf');

  insert into arc_test_results
  select '14 failed intent with a leftover pending object is cleaned',
         exists (select 1 from public.storage_deletion_queue
                 where storage_object_path = 'pending/failed.pdf');
  ...
  select '15 a finalized upload and its document are never removed by cleanup',
     ...
     and not exists (select 1 from public.storage_deletion_queue
                     where storage_object_path = 'documents/finalized-upload.pdf')
  ...
  select count(*) into n from public.storage_deletion_queue
   where storage_object_path = 'pending/stale.pdf';
  ...
  select '19 expired guest with documents queues its files before relational deletion',
     ...
     and exists (select 1 from public.storage_deletion_queue
                 where storage_object_path = 'documents/guest-owned.pdf'
  ...
  select '21 a migrated document survives expiry of its former guest workspace',
     ...
     and not exists (select 1 from public.storage_deletion_queue
                     where storage_object_path = 'documents/migrated.pdf');
...
begin
  ...
  insert into public.source_documents (contract_id, storage_object_path, original_filename,
                                       display_name, sha256, byte_size, page_count)
  values (contract_id, 'documents/draft-doc.pdf', 'd.pdf', 'Draft PDF', repeat('4', 64), 100, 1)
  ...
  select '24 Delete Draft queues its stored files before deleting the hierarchy',
     ...
     and exists (select 1 from public.storage_deletion_queue
                 where storage_object_path = 'documents/draft-doc.pdf'
                   and completed_at is null)
  ...
  insert into public.source_documents (contract_id, storage_object_path, original_filename,
                                       display_name, sha256, byte_size, page_count)
  values (contract_id, 'documents/history-doc.pdf', 'h.pdf', 'History PDF', repeat('5', 64), 100, 1)
  ...
  select '25 Discard amendment draft preserves the contract documents',
     ...
     and not exists (select 1 from public.storage_deletion_queue
                     where storage_object_path = 'documents/history-doc.pdf')
