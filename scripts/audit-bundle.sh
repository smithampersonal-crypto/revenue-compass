...
FOUND=0
for pattern in 'SUPABASE_SERVICE_ROLE_KEY' 'sb_secret_[A-Za-z0-9_-]{8,}' '"role" *: *"service_role"' 'cm9sZSI6InNlcnZpY2Vfcm9sZ' 'supabaseAdmin' 'client\.server' 'pdfjs-dist' 'GlobalWorkerOptions' 'getDocument\(\{ *data' 'validation\.server' 'pdf-parser\.server' 'ARC_MAINTENANCE_SECRET' 'arc_run_maintenance' 'arc_claim_storage_deletion_jobs' 'store\.server' 'caller\.server' 'maintenance\.server' 'runs\.store\.server' 'autosave\.store\.server' 'arc_create_ai_run' 'arc_reserve_ai_allowance' 'arc_mark_ai_run_failure' 'arc_affirm_ai_review_item' 'arc_resolve_ai_review_issue' 'arc_acknowledge_ai_stale_sources' 'arc_save_draft_with_ai_reconciliation' 'arc_ai_source_set_fingerprint' 'OPENAI_API_KEY' 'openai\.server' 'evidence\.server' 'request-package\.server' 'terra\.server' 'responses\.create' 'phase9d-smoke' 'authorized-sources\.server' 'data:application/pdf;base64' 'exceljs' 'Revenue_Compass_ASC606_Master_Library'; do
