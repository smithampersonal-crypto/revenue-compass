...
function policySection(): string {
  return [
    ...
    '- Use evidenceMode "text" for running prose evidence. You never write excerpt text: ARC owns the excerpt. Instead select ARC anchors. Set anchorStart and anchorEnd to anchor ids taken from the ARC local citation text mirror in Section 4, and ARC materialises the exact quotation from its own extraction of that page.',
    "- Select the SMALLEST contiguous anchor range that carries the supporting language. A single anchor is normal; the range may never exceed 3 consecutive anchors.",
    "- anchorStart and anchorEnd must be real anchor ids that ARC supplied for exactly one physical page, in forward order, and pageStart and pageEnd must both equal that physical page.",
    ...
    '- Use evidenceMode "visual" where there is no running sentence to anchor: tables, pricing or SLA grids, column/row relationships, signature blocks, figures, page layout and wide letter-spaced display banners. A visual citation MUST set anchorStart and anchorEnd to null, and is fully acceptable evidence there.',
    '- For evidenceMode "text" both anchors are required and neither may be null; for evidenceMode "visual" both must be null. A citation that breaks this contract is rejected and the whole analysis fails.',
