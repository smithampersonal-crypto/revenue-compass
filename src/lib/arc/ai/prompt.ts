...
function taskSection(promptVersion: string, outputSchemaVersion: string): string {
  return [
    ...
    "- report fixedConsiderationInput as the exact fixed amount the evidence states, and describe its period in the rationale (for example an annual fee on a multi-year term). Never multiply, annualize, pro-rate or otherwise compute a full-term total: ARC derives the contract total from the billing schedule and the service period itself;",
