...
function policySection(): string {
  return [
    ...
    "- anchorIds is an explicit ordered list of every cited text segment, NOT a [start, end] range. A text citation may contain only 1 anchor, or 2 to 3 adjacent consecutive anchors.",
    "- Never provide only the first and last anchors of a longer section. Never skip intervening segment ids. Never omit an intermediate anchor. Never cite more than 3 text anchors for one citation.",
    '- For example, to cite P0001-S0015 through P0001-S0017, return ["P0001-S0015","P0001-S0016","P0001-S0017"]. Returning only ["P0001-S0015","P0001-S0017"] is invalid.',
    '- Valid: ["P0003-S0011"]. Valid: ["P0003-S0009","P0003-S0010"]. Invalid: ["P0003-S0008","P0003-S0011"], because it skips P0003-S0009 and P0003-S0010 and incorrectly treats anchor ids as range endpoints.',
    '- For negative or absence conclusions such as no noncash consideration or no significant financing component: prefer the smallest specific text anchor that directly supports the conclusion when one exists; if the conclusion depends on reviewing a broader page or section rather than one direct textual statement, use evidenceMode "visual" with anchorIds: []; never manufacture a start/end text range to represent broad-page evidence.',
...
function taskSection(promptVersion: string, outputSchemaVersion: string): string {
  return [
    ...
    "- Do not create a separate performance obligation merely because a contract separately prices or labels a volume, capacity, tier, quota, entitlement, or usage allowance. First determine whether it transfers an additional good or service independently from the underlying service, or merely defines the quantity, capacity, access level, or included usage of that underlying service. If it only defines how much of the underlying service the customer may consume and does not transfer an additional independently identifiable good or service, group it with the underlying service rather than creating a separate performance obligation. When the evidence does show that such an item transfers an additional distinct good or service, conclude it is a separate performance obligation and say which terms show that;",
    ...
    "- A separately stated price does not by itself establish a separate performance obligation. If a separately stated amount prices a volume, capacity, tier, quota, entitlement, or usage allowance that is grouped with an underlying performance obligation, include that amount in the provisional SSP of the underlying performance obligation rather than creating a separate SSP item solely because the amount appears on a separate contract line;",
