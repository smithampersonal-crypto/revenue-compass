...
- Animate only a localized light, translucent activity front inside the current stage span using narrowly scoped CSS. It will sweep, fade out before reset, and repeat without growing from the stage start or moving the confirmed boundary.
...
- Under reduced motion, disable both the sweep and confirmed-fill transition, and show a small static cue near the confirmed boundary rather than tinting the full unconfirmed stage span.
